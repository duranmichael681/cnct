      ...
      <div className='flex flex-1'>
        ...
        <div className='flex-grow flex flex-col lg:flex-row pb-24 md:pb-0 md:ml-[70px]'>
          ...
          <div className='p-4 sm:p-6 lg:p-8 w-full lg:w-2/3 flex flex-col gap-6 sm:gap-8 lg:gap-10'>
            ...
            {/* Loading State */}
            {loading && <LoadingSpinner message="Loading posts from Supabase..." />}
            ...
            {!loading && !error && displayPosts.length === 0 && (
              <div className='text-center py-10 bg-[var(--menucard)] rounded-lg'>
                ...
                <p className='text-[var(--text)] opacity-70 mt-2'>Be the first to create a post!</p>
              </div>
            )}

            {/* Posts List */}
            {!loading && displayPosts.map((post, index) => (
              <div key={post.id} className={`animate-fade-in-delay-${Math.min(index + 2, 4)}`}>
                <div className='bg-[var(--menucard)] rounded-lg p-6 shadow-lg hover:shadow-xl transition-shadow'>
                  ...
                  <div className='flex gap-4 text-sm text-[var(--text)] opacity-80 flex-wrap'>
                    ...
                    {post.end_date && <span>⏰ Ends: {formatEventDate(post.end_date)}</span>}
                    <span>👥 {getAttendeeCount(post)} attending</span>
                    {post.is_private && <span className='text-yellow-500'>🔒 Private</span>}
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className='m-5 w-1/3 h-auto flex flex-col justify-start items-center border-l border-[var(--border)] mt-20 animate-fade-in-delay-1'>
              ...
              <div className='flex flex-col w-full gap-10'>
                {popularPosts.length === 0 ? (
                  <p className='text-center text-[var(--text)] opacity-70'>No posts yet</p>
                ) : (
                  popularPosts.map((post, index) => (
                    <div key={post.id || index} className={`flex w-full relative cursor-pointer hover:bg-[var(--menucard)] transition-colors rounded-lg p-2 animate-fade-in-delay-${Math.min(index + 3, 4)}`}>
                      <div className='flex flex-col sm:flex-row w-full items-center sm:items-start sm:justify-start'>
                        <img src={post.image} alt={post.title} className='ml-5 rounded-xl max-w-[175px] aspect-square object-cover mt-10' />
                      </div>
                      <div className='mt-10 mr-10 w-2/3 flex-row items-center justify-center'>
                        <h2 className='text-[var(--primary)] font-semibold text-2xl'>{post.title}</h2>
                        <div className='bg-[var(--border)] h-0.25 w-15 my-3' />
                        <p className='text-[var(--text)] line-clamp-3'>{post.description}</p>
                        <p className='text-[var(--text)] opacity-70 text-sm mt-2'>📍 {post.location}</p>
                        <p className='text-[var(--text)] opacity-70 text-sm'>📅 {post.date}</p>
                      </div>
                    </div>
                  ))
                )}
